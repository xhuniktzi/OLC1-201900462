...
export class DoUntil implements IStatement {
  constructor(
  ...
  graph(): string {
    let str: string = `node${this.line}${this.column}[label="DoUntil"];`;
  ...
  execute(sym_table: SymbolTable): void {
    do {
      try {
        const while_table: SymbolTable = new SymbolTable(sym_table, "do-until");
